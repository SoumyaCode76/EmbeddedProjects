...
/* Project headers: bring in HAL types and logger API */
#include "usart.h"
#include "log_uart.h"
...
{
  (void)file;
  /* Try to send the whole buffer via UART to reduce per-char overhead.
     This assumes a working __io_putchar implementation or HAL UART handle.
     If huart1 is available, prefer HAL_UART_Transmit for the full buffer. */
  if (len <= 0 || ptr == NULL) return 0;

  /* Prefer non-blocking logger if available */
  {
    extern size_t LOG_UART_Enqueue(const uint8_t *data, size_t len);
    size_t written = LOG_UART_Enqueue((const uint8_t *)ptr, (size_t)len);
    if (written > 0) return (int)written;
  }

  /* Fallback: try HAL transmit directly */
  extern UART_HandleTypeDef huart1; /* declared in usart.c */
  HAL_UART_Transmit(&huart1, (uint8_t *)ptr, (uint16_t)len, HAL_MAX_DELAY);
  return len;

  /* Last resort: per-char via __io_putchar */
  for (int i = 0; i < len; ++i)
  {
